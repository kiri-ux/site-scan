...
srv = http.server.ThreadingHTTPServer(("127.0.0.1", 8917), handler)
...
r = c.post("/scan", json={"url": "http://localhost:8917/onetrust.html", "full": True})
d = r.get_json()
print("scan full:", d["verdict"], d["mode"], [x["name"] for x in d["cmps"]],
      "gtm:", d["gtm"]["container_ids"])

r = c.post("/scan", json={"url": "http://localhost:8917/cookiebot.html", "full": False})
d = r.get_json()
...
r = c.post("/scan", json={"url": "http://localhost:8917/nocmp.html", "full": True})
d = r.get_json()
print("nocmp:", d["verdict"], "violations:",
      [h["vendor"] for h in d["pre_consent"] if h["severity"] == "violation"])
